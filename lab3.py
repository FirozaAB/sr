...
IPCONFIG_FILE = '/mnt/hgfs/A3/cs144_lab3/IP_CONFIG'
IP_SETTING={}
...
# Set sw0-eth3's MAC to match the host's primary NIC (or a fixed fallback)
    dev = quietRun("ip route get 8.8.8.8 | awk '/dev/{for(i=1;i<=NF;i++) if($i==\"dev\") { print $(i+1); exit }}'").strip()
    if not dev:
        dev = quietRun("ip -o link | awk -F': ' '$2!=\"lo\"{print $2; exit}'").strip()
    hw = quietRun("cat /sys/class/net/%s/address" % dev).strip()
    if hw:
        info('*** setting mac address of sw0-eth3 the same as %s (%s)\n' % (dev, hw))
        router.intf('sw0-eth3').setMAC(hw)
    else:
        info('*** could not determine host MAC; using a fixed MAC 02:00:00:00:00:01\n')
        router.intf('sw0-eth3').setMAC('02:00:00:00:01')
